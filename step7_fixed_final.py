...
def pick_best_scenario(df: pd.DataFrame, scenario_cols: List[str], num_classes: Optional[int]=None,
    ...
    # conflict_penalty (ΣΥΓΚΡΟΥΣΗ) ως 2ο κριτήριο — hard constraint πριν broken friendships
    pool_sorted = sorted(
        scores,
        key=lambda s: (
            s["total_score"],
            int(s["conflict_penalty"]),         # μηδέν συγκρούσεις → προτεραιότητα
    ...
    ties = [s for s in pool_sorted if (
        ...
        int(s["conflict_penalty"]) == int(head["conflict_penalty"]) and
